...
public class Thread02Main {
	...
	public static void main(String[] args) {
		...
		// 3. Runnable을 구현하는 클래스의 인스턴스를 생성
		Runnable runnable1 = new MyRunnable("thread1");
		Runnable runnable2 = new MyRunnable("thread2");
		Runnable runnable3 = new MyRunnable("thread3");
		...
		new Thread(new MyRunnable("thread4")).start();
		...
		System.out.println("-- main()(main thread)종료  -- ");
	} // end main()
...
class MyRunnable implements Runnable {
	...
	public void run() {
		for (int i = 0; i < 100; i++) {
			// Thread.currentThread() - 현재 실행중인 쓰레드 리턴
