...
public class PhonebookController implements Controller, C, Closeable {
	...
	// 데이터
	//                 <uid, Model>
	private Map<Integer, PhonebookModel> pbMap = 
			new TreeMap<Integer, PhonebookModel>();
	
	// 추가할 멤버변수 (필요하다면)
	private static int getUid = 0;	// 새로 생성되는 데이터에 부여할 Uid값 
	...
	private int getMaxUid() {
		if(pbMap.size() == 0) return 0;
		
		// List<> 로 저장되는 경우 가장 마지막 요소의 Uid값이 최대값
		return pbMap.get(pbMap.size() - 1).getUid();
	} 
	...
	private int findIndexByUid(int uid) {
		...
		for(int index = 0; index < pbMap.size(); index++) {
			if(pbMap.get(index).getUid() == uid){
				return index;
			}
		}	
